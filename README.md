...
Experimental v0.4.1 CLIProxyAPI provider for Cursor subscription models through the official `agent` CLI only.
...
- Streaming requests run the official CLI with `agent --print --output-format json --sandbox disabled` and return buffered, protocol-correct SSE frames after the terminal JSON result is validated. Bounded file captures prevent Cursor's detached worker-server from keeping stdout/stderr pipes open after the command exits.
...
Cursor quota visibility is integrated through CLIProxyAPI's native auth/model path, not a plugin-owned quota page. Parsed Cursor auth records return both `Auth` and `Auths`, include OAuth-style account metadata, and bind per-auth models through `ModelsForAuth`; CPA then shows the Cursor credential in `/v0/management/auth-files` and the built-in Quota Management UI.

Only safe fields from official Cursor CLI output are copied into auth metadata (`userEmail`/account, `subscriptionTier`, and `cliVersion`/version). Numeric remaining subscription quota is unavailable because the official `agent about` JSON does not expose it. CPA can still show observed request/failure health, recent request buckets, and real quota cooldowns detected while routing Cursor requests. The plugin does not read Cursor OAuth/token files and does not call private Cursor endpoints.
...
dist/cliproxyapi-cursor_0.4.1_linux_amd64.zip
dist/checksums.txt
```
...
python3 - <<'PY'
import zipfile
with zipfile.ZipFile('dist/cliproxyapi-cursor_0.4.1_linux_amd64.zip') as z:
    print(z.namelist())
PY
sha256sum dist/cliproxyapi-cursor_0.4.1_linux_amd64.zip
```
